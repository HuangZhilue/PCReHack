...
{
    public class RectLeftViewModel : Screen, IHandle<string>
    {
        ...
        //public BitmapSource Image { get; set; } = Resource.HeadLeft.BitmapToBitmapSource();
        //public Bitmap ImageBitmap { get; set; } = Resource.HeadLeft;
        public bool IsFound { get; set; } = false;

        public RectLeftViewModel(IEventAggregator eventAggregator)
        {
                ...
                while (IsStart)
                {
                    try
                    {
                        ...
                        foreach (TempImage temp in TempImageList)
                        {
                            ...
                            Debug.WriteLine($"Left:\t{maxValues[0]}");
                            if (maxValues[0] > 0.40)
                            {
                                IsFound = true;
                            }
                        ...
                        #region 保存图片
                        //string outputFileName = System.AppDomain.CurrentDomain.BaseDirectory + "//testLeft.jpg";
